...
import { processFile, getFileDownloadUrl, getChartDownloadUrl, downloadFile } from "../services/api";
...
export function PromptToolSection() {
  ...
  const handleDownloadChart = async () => {
    if (result?.chart_url) {
      ...
      await downloadFile(`http://localhost:8000${result.chart_url}`, filename);
    }
  };
                      ...
                      {result.processed_file_url && (
                        <Button
                          onClick={() => {
                            ...
                            downloadFile(`http://localhost:8000${result.processed_file_url!}`, filename);
                          }}
                          variant="outline"
