...
export function SheetViewer({ data, columns, rowCount, onDownload, highlightDuplicates = true }: SheetViewerProps) {
  ...
  return (
    <div className="h-full w-full bg-white border-2 border-gray-200 rounded-xl overflow-hidden shadow-lg transition-all duration-300 ease-in-out hover:shadow-2xl hover:border-[#00A878] flex flex-col">
      {/* Header */}
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  {columns.length > 5 && (
                    <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 border-b border-gray-200 border-r-2 border-gray-300">
                ...
                {data.slice(0, 5).map((row, rowIdx) => {
                  ...
                  return (
                    ...
                    >
                      {columns.slice(0, 5).map((col, colIdx) => {
                        ...
                        return (
                          <td 
                            key={colIdx}
                            style={cellStyle}
                            className={`px-3 py-2 border-r-2 border-gray-300 last:border-r-0 ${
                              isDuplicate && !hasFormatting
                                ? 'text-yellow-900 font-medium bg-yellow-50' 
                      ...
                      {columns.length > 5 && (
                        <td className={`px-3 py-2 text-xs border-r-2 border-gray-300 ${
